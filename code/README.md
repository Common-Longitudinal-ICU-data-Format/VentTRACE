...
| `03_context.py` | For each index paralytic: does the ventilator record show a device transition onto IMV within configured ±60 min, and was a sedative charted within configured ±5 min, at what dose; D.2 separately plots the nearest transition within ±6 h. | `step01__cohort_resp_waterfall.parquet`, `medication_admin_intermittent` filtered to the sedatives | `output/intermediate_phi/step03__index_context.parquet` (PHI); `step03__*.csv`, paired `fig_D1`–`fig_D2`, `fig_E1`–`fig_E3` data/PNGs |
| `04_covariates.py` | Owns the analytic row, block-first hospital/year counts, the separate corrected 28-day dose-weight selector, normalized ECDFs, induction percentiles/tiers, and their eligibility flow. | `patient`, `vitals`, `medication_admin_continuous`, `crrt_therapy`, `hospital_diagnosis`, `labs`, `patient_assessments`, and re-opens `hospitalization`, `adt`, and respiratory support through `compute_sofa_polars` | `output/intermediate_phi/step04__index_covariates.parquet` (PHI); `step04__*.csv`, paired `fig_B2`, `fig_E4`, `fig_E5`, `fig_G1`, and `fig_T2` data/PNGs |
| `05_table_one.py` | Restricts Table 1 to valid indexes with both an IMV transition and sedation; publishes all valid events and one first-valid event per block in a human-readable CSV and aggregation JSON. | nothing — reads `step04__index_covariates.parquet` | stable `table1_by_agent_{block,index}_readable.csv` and `.json`; paired `fig_T1` and `fig_T2` outputs |
