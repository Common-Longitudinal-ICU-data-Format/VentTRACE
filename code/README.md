...
| `03_context.py` | For each index paralytic: does the ventilator record show a device transition onto IMV within configured ±60 min, and was a sedative charted within configured ±5 min, at what dose. | `step01__cohort_resp_waterfall.parquet`, `medication_admin_intermittent` filtered to the sedatives | `output/intermediate_phi/step03__index_context.parquet` (PHI); `step03__*.csv`, paired `fig_D1`, `fig_E1`–`fig_E3` data/PNGs |
