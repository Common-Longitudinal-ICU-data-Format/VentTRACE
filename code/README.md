...
`utils/suppress.py` is the only route for analysis artifacts in `output/final_no_phi/` —
`publish()` for CSV and `publish_json()` for aggregation payloads, both refusing to write a frame
carrying an identifier or datetime column through one shared check. The launchers separately write
console output to `output/final_no_phi/logs/`; those logs are not processed by `publish()` or
inventoried by the artifact manifest. See [`../docs/pipeline_flow.md`](../docs/pipeline_flow.md) §6.
