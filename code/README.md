...
| `05_table_one.py` | Table 1, published twice from one row inventory (by encounter block at its first index, and by index event) and in three forms each: the long numeric CSV, a human-readable CSV, and a JSON for cross-site aggregation. Every row carries its own rule and unit. | nothing — reads `index_covariates.parquet` | `table1_by_agent_{block,index}.csv`, `…_readable.csv`, `…​.json`, `figures/T1_*.png`, `figures/T2_*.png` |
