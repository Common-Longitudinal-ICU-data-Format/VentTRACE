...
**Cause 2 — no agent-event collapse.** Co-administration of two different same-class agents is a Δ≤1 min phenomenon. Measured in-pipeline and published as `pair_collapse_deltas.csv`: at Δ=0 there are **4,615** different-agent intervals against **301** same-agent redoses (15.3×), at Δ=1 **883** against **223** (4.0×), and from Δ=2 onward the two series run at the *same* rate (6,127 vs 6,952, 0.88×) — no co-administration signal is left, only routine dosing. **95.3% of the positive excess — every bin where different-agent exceeds same-agent — sits at Δ≤1.** D28's "step over, don't consume" already prevents that from making extra pairs, so collapsing sedatives alone is a ~0.4% no-op. The residual multi-pair episodes are driven by **repeat paralytic dosing** — `108083_E3` has five vecuronium doses across 50 minutes. Those separations have no empirical valley, so the threshold is a **clinical definition, not a fit to the data**, and must be documented as such. (The pre-collapse figures that motivated this — median 209 min, 12% within 15 min — came from the same scratchpad, and the same wrong timezone conversion, as the Δ counts above; treat them as indicative only. The verifiable post-collapse fact: 197 episodes still carry more than one pair, and the paralytics driving them sit a median **470 min** apart, with 10.6% within 30 min and none within 15 by construction.)
...
**Verification:** `output/final_no_phi/` contains exactly 36 artifacts (25 CSV + 11 PNG) and the manifest assertion passes in both directions.
