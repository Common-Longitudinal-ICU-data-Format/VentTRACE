...
| Column | Type | Notes |
|---|---|---|
| `encounter_block` | Int64 | the scan's unit |
| `patient_id` | str | |
...
Alongside the two ordinary artifacts, `SED` writes **two** D40 sweep files, both to `output/intermediate_phi/` and both over the threshold grid `5, 10, 15, 30, 45, 60, 90, 120, 150, 180` minutes. `method_SED_prep_sweep.parquet` is the pooled curve, one row per (threshold, `index_class`), carrying `n_episodes`, `n_detected_all`, `n_detected`, `n_flipped` and `n_doses_reclassified`; `07` publishes it as F.2. `method_SED_prep_by_drug.parquet` is the same grid broken out **by `med_category`**, one row per (threshold, `index_class`, `med_category`), and it counts post-t₀ *administrations* rather than episodes — `n_doses_after` and `n_doses_reclassified`; `07` reads it at F.3. The configured `infusion_prep_minutes` is one point on that grid and carries no special status in either file; §8.2 publishes the curves so the threshold can be chosen against evidence rather than defended from first principles.
...
**D40 and D41 apply here identically**, including `method_PARA_prep_sweep.parquet` and `method_PARA_prep_by_drug.parquet` over the same grid and with the same schemas as §7.1. The effect at MIMIC is small — 584 detected episodes fall to 553 at the 60-minute threshold, and `during_infusion` reaches 18 of 588 qualified ranked entries — but the rule is not carried here for its effect. It is carried because `SED` and `PARA` must mean the same thing by "an administration in the window", and a site that runs cisatracurium or vecuronium infusions for ventilator dyssynchrony will see a materially larger one. Applying it to only one method would make the two methods' `detected_induction_only` columns non-comparable: Tier F sets `SED` and `PARA` side by side in F.1 and on one axis in F.2 and F.3, and a refinement carried by one and not the other would put two different readings of "an administration in the window" in adjacent rows of the same table. **Tier A is not what is at stake** — under D42 neither method's `detected` moves at all, which is exactly why the symmetry has to be argued from Tier F rather than from the cross-method tables.
...
> **`SED` and `PARA` are not temporally folded, and do not need to be.** The fold is `PAIR`'s alone — `03` and `04` never call it — and that asymmetry is deliberate rather than an omission. §6.2 already collapses their repeats, by a *different* rule: deduplication **by `med_category`**, keeping the single administration nearest t₀. That is the correct collapse for a t₀-anchored ranking, where the question is "which agent, and how close to the intubation", and a redose two minutes later changes neither answer. `PAIR` has no t₀ to rank against (D27, D30) and counts *pairings* rather than proximities, so its repeats have to be merged **temporally** or they pair a second time — which is exactly what D43 exists to stop. The asymmetry cannot reach a published cross-method table: every one of them keys on the boolean `detected`, and the only artifact where a `med_category` value crosses into a shared column, `timing_by_medication.csv`, is `SED`/`PARA` only — its `med_category` therefore carries single agents exclusively and a combined `PAIR` label can never sit in a column beside one of its own components.

> **`PAIR` has no `detected_induction_only`, and correctly does not need one.** The D40 reclassification and the D41 flag are `SED`'s and `PARA`'s alone — `05_method_pair.py` names neither `infusion_prep` nor `during_infusion` nor `medication_admin_continuous`, and its scan consumes every qualifying intermittent administration raw, including the post-t₀ boluses D40 reclassifies for the other two methods. Stated plainly: **a `PAIR` detection may rest on a pair whose sedative member is a maintenance-prep bolus.** That is the fold asymmetry of the note above running in the other direction, and D42 is what makes it correct rather than an omission. D42 holds the refinement out of every column any other tier reads, so there is nothing for `PAIR` to be inconsistent *with*: Tier F is the only consumer of `detected_induction_only` and F.1–F.4 loop over the ranked methods only, so `PAIR` is never asked for a column it does not have; Tier A keys on the unrefined `detected` for all three methods; and the `PARA` × `PAIR` integrity check in §8 reads the unrefined `para_detected`, so D40 cannot have moved it. Refining `PAIR` would also mean refining *half* of a pair — its paralytic member carries its own D40 status — and would put a fourth definition of detection into a study that already publishes two.

> **The fold costs the scan up to `collapse_gap_minutes` of reach — it is not detection-neutral.** An event is dated by its **earliest** administration (D43.4), so the event's later rows are invisible to the `pair_gap_hours` test: a sedative row 179 minutes before a paralytic can no longer pair with it once it has been folded backwards into an event anchored 15 minutes earlier, because the scan measures from the anchor. The note below argues only the direction the `04` comparison needs — that a paralytic never moves *later* — and a reader could wrongly infer from it that the fold cannot destroy a pair at all. It can, at the far edge of the 180-minute window, and the measured cost at MIMIC is **exactly one episode: 1,273 episodes with at least one pair become 1,272**. That is the whole of it, and it is recorded because "one" is a number a reader can check rather than a claim they have to take.
...
|---|---|---|---|---|---|
| all qualified pairs | 1 407 | 3.0 | 1.0 … 11.0 | 62.4 | 88.1 |
...
|---|---|---|---|---|---|
| propofol | rocuronium | 243 | 0.0 | 50.0 | mg |
| fentanyl | rocuronium | 102 | 72.0 | 50.0 | mcg |
| fentanyl+propofol | rocuronium | 95 | 14.0 | 50.0 | mcg |
| midazolam | rocuronium | 74 | 15.5 | 2.0 | mg |
| fentanyl | vecuronium | 57 | 59.0 | 50.0 | mcg |
| fentanyl+midazolam | vecuronium | 41 | 20.0 | 50.0 | mcg |
| … | | | | | |
| fentanyl+midazolam+propofol | rocuronium | 15 | 8.0 | 100.0 | mcg |

**The `+` rows are the shape D43.5 produced, and the shape is not illustrative even where the values are.** A `sed_med_category` may be a *combined* label — one agent event whose fold caught more than one agent, alphabetically sorted and joined with `+` — and it is a distinct row from either component alone, because giving both together is a different induction from giving either. Half the published rows carry one at MIMIC. The grid is therefore wider than the `SED` list is long, and a reader who expects one row per single agent will mis-read it. Two further consequences to read with the table: the row order is by `n`, so the combined rows are not confined to the bottom; and `median_sed_dose` is the dose of the label's **lead** agent, taken from that agent's earliest administration in the event (D43.6) — never a total across a combination, and on any folded row, combined or not, it describes one administration rather than the event.

This is the clinical output the method exists to produce, and it is not derivable from `SED` and `PARA` run separately — those two report their marginals, never the joint. Rows with long median gaps are the ones most likely to be co-occurrence rather than a deliberate induction pair, and their share is a direct read on `SED`'s list breadth (D10).
...
| Directory | Contents |
|------------------------------------|------------------------------------|
| `output/intermediate_phi/` | `cohort.parquet`, `cohort_resp_waterfall.parquet`, `cohort_resp_imv_raw.parquet`, `cohort_index.parquet`, `cohort_qc.csv`, `index_imv.parquet`, `method_{SED,PARA}_ranked.json`, `method_{SED,PARA,PAIR}_episode.parquet`, `method_PAIR_pairs.parquet`, `method_{SED,PARA}_prep_sweep.parquet`, `method_{SED,PARA}_prep_by_drug.parquet`, `pair_collapse_deltas.parquet`, `reference_cpt.parquet` |
...
The four sub-analysis parquets, added by D40 and never previously enumerated here, are:

| File | Written by | Contents |
|---|---|---|
| `method_SED_prep_sweep.parquet` | `03` | D40 recomputed over the ten-point threshold grid, one row per (`threshold_minutes`, `index_class`): `n_episodes`, `n_detected_all`, `n_detected`, `n_flipped`, `n_doses_reclassified`. Read by `07` for F.2 |
| `method_PARA_prep_sweep.parquet` | `04` | the same, for `PARA`; identical schema, so `07` concatenates the two |
| `method_SED_prep_by_drug.parquet` | `03` | the same grid broken out by `med_category`, one row per (`threshold_minutes`, `index_class`, `med_category`), counting post-t₀ administrations rather than episodes: `n_doses_after`, `n_doses_reclassified`. Read by `07` for F.3 |
| `method_PARA_prep_by_drug.parquet` | `04` | the same, for `PARA`; identical schema |

**Nothing asserts a manifest over `output/intermediate_phi/`.** The schema gate at the end of `07` covers `output/final_no_phi/` only, so an artifact added here and left out of this table is not caught by any run — which is how these four went unlisted through two sessions. A file added to the PHI layer has to be added to this table by hand.
