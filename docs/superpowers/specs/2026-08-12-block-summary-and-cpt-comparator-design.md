...
**SOFA category boundary.** `compute_sofa_polars` reopens labs, vitals, patient assessments, respiratory support, and continuous medications and applies its own exact category filters. Step 04 therefore stages temporary, cohort-scoped parquet inputs with stripped/canonical categories before calling clifpy. Respiratory devices are mapped to the title/uppercase values clifpy's SOFA implementation expects; the temporary directory is removed when computation finishes.
