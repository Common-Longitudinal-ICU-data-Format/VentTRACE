...
**Loading (P19, restated because this amendment adds seven tables).** Every new raw table is read through its clifpy table class via `from_file`. Large category-scoped reads may push down explicit canonical and supported alias/casing values (P20), then canonicalize and filter the returned values again in memory. clifpy normalizes recognized datetime columns to `TIMEZONE` before the dataframe reaches analytic transformations; `to_site_naive` then removes that timezone with `series.dt.tz_localize(None)`, preserving the site-local wall clock without a second conversion. The respiratory waterfall in `01` is the only UTC boundary. Timezone conversion is never done in polars, whose DST table stops extrapolating US rules around 2099 while MIMIC's dates are shifted into the 2100s.
...
**SOFA category boundary.** Step 04 first loads labs, vitals, patient assessments, respiratory support, and continuous medications through their clifpy table classes with `TIMEZONE`. It then stages temporary, cohort-scoped parquet inputs with stripped/canonical categories for `compute_sofa_polars`, whose exact category filters require those canonical values. Respiratory devices are mapped to the title/uppercase values clifpy expects; the temporary directory is removed when computation finishes.
