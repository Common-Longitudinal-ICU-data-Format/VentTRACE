...
| P37 | **Mortality is `death_dttm` falling inside a member hospitalization's admission→discharge interval, OR `discharge_category == 'expired'`. ICU mortality additionally requires `death_dttm` to fall inside an ADT `icu` interval; blocks flagged dead by `discharge_category` alone are published as `icu_mortality_undeterminable`.** | Set by the study lead. The bound on `death_dttm` is not decoration: in CLIF the `patient` table's `death_dttm` can be registry-sourced and will fire for a patient discharged alive who died months later at home, which unbounded would be published as in-hospital mortality. ICU attribution by death time is more precise than by last-known location, and its cost — undeterminable for category-only deaths — is published as its own count rather than absorbed into either numerator. |
...
block-level   n_index_in_block · los_hospital_days · los_icu_days
              hospital_mortality · icu_mortality · icu_mortality_undeterminable
```
...
outcomes       hospital_mortality · icu_mortality · icu_mortality_undeterminable
...
| `tests/test_mortality_bound.py` | P37: a `death_dttm` after discharge does not count as in-hospital mortality; a category-only death lands in `icu_mortality_undeterminable`, not in either numerator. |
