...
| P42 | **Rocuronium and succinylcholine charted as `mg/kg` are treated as mislabeled absolute `mg` doses for calculation at every site.** The raw `mg/kg` value and unit remain unchanged in `paralytic_dose_units.csv` and the P41 ECDF; `paralytic_dose_unit_corrections.csv` publishes the affected administration count per agent. Every other `/kg` unit still fails. Rows with a null/non-finite dose or null unit remain in administration and raw-unit counts but are excluded, with a logged count, from converted dose statistics. | Set by the study lead after a consortium run observed median charted values of 49.95 `mg/kg` for rocuronium and 96 `mg/kg` for succinylcholine. Interpreting those literally and multiplying by a typical adult weight would produce multi-gram paralytic doses, while the values themselves are conventional absolute-mg doses. The correction must be visible rather than silently rewriting the source, and it must not become permission to discard the weight axis from any other medication or unit. |

P42 supersedes P18's earlier statement that every paralytic `/kg` unit requires vitals. That remains the rule for every unit except the two exact P42 category/unit pairs.
...
| `paralytic_dose_unit_corrections.csv` | the two P42 `(med_category, mg/kg → mg)` rules → n affected administrations, including an explicit zero when absent |
