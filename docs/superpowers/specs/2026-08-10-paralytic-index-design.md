...
3. **Does the ventilator record show a transition onto invasive ventilation around the index paralytic?** Not "was IMV charted" — a patient already ventilated satisfies that without anything having happened — but *did the device change*, from 30 minutes before through 60 minutes after the index.
...
| P20 | **Every loaded source `*_category` value is stripped and lower-cased before matching, joining, or grouping; every categorical literal is lower case.** Large category-scoped reads may pass an explicit list of canonical and supported alias/casing values to clifpy `from_file` so projection, filtering, and timezone handling stay inside the same source boundary. The returned values are still canonicalized and filtered again in memory. The initial whole-site medication scan uses the equivalent normalized DuckDB pushdown exposed by clifpy's lazy IO. | Category pushdown prevents weight-only and SOFA reads from materializing tens of millions of unrelated rows. Explicit aliases cover the supported source spellings; post-load normalization remains the authoritative analytic representation. A source spelling outside that list is a vocabulary-contract issue rather than a clinical zero. |
...
| P48 | **Figure D.2 is a fixed ±6-hour sensitivity view of the same P12 transitions, using one nearest transition per index paralytic and 30-minute bins.** An exact-distance tie selects the earlier transition. D.2 does not modify the configured -30/+60-minute D.1 detector, persisted `imv_transition`, or downstream cohort definitions. | Set by the study lead to show where a device transition falls beyond the primary window without turning IMV state rows into transitions or changing the primary analysis. One selected event per index preserves the D.1 counting unit; nearest selection answers temporal proximity over the wider window. |
| P49 | **AMENDED 2026-08-19: strict medication dose bounds are required in `config["medication_dose_upper_bounds"]` and are part of medication eligibility alongside action and unit.** Eligible configured absolute-unit rows satisfy `0 < dose < bound`: etomidate 200 mg, fentanyl 500 mcg, ketamine 100 mg, midazolam 50 mg, propofol 500 mg, rocuronium 400 mg, succinylcholine 400 mg, and vecuronium 30 mg. Configured `/kg` rows remain exempt from absolute bounds but must be positive and finite. Ineligible paralytics do not form indexes; ineligible sedatives do not satisfy sedation context or enter dose artifacts. | Set by the study lead after reviewing the observed distributions. Ketamine's strict 100 mg bound is the observed UCMC p95 and intentionally excludes values equal to 100 mg for consistency with every other strict boundary. This supersedes P43's summary-only scope and P47's action/unit-only eligibility definition. |
| P50 | **Figure 1 is the main analysis CONSORT and ends at Table 1.** It starts with every qualifying paralytic administration, shows the 15-minute formed index and within-index same-agent merge, then applies the procedural-location, persisted IMV-transition, and sedation gates before splitting into all valid index events and one first-valid index per block. Every stage carries source-administration, post-merge medication-entry, index, and block counts; every gate carries the removed counts and reason. Subanalyses do not alter this flow. | Set by the study lead so the index denominator and its losses are visible in one primary artifact rather than reconstructed across subanalysis outputs. Step 05 aggregates existing fields only: it does not duplicate medication, location, IMV-window, or sedation-window logic. |
| P51 | **AMENDED 2026-08-19: formed indexes whose anchor is in an ADT `location_category == procedural` interval are excluded before downstream context.** The anchor is `t_dttm`; interval membership is half-open (`in_dttm <= t_dttm < out_dttm`) with null `out_dttm` open-ended. Overlaps use earliest `in_dttm`, then category and hospital ID. Missing coverage is retained as `unknown`. Exclusion occurs after the 15-minute fold, so Figure A.1 remains unchanged; retained indexes are renumbered contiguously within each block. | Set by the study lead. Classifying the formed anchor matches the existing `location_at_index` definition without allowing a later member administration to reclassify the event. The aggregate exclusion summary and Figure 1 gate retain the formed-to-excluded-to-retained accounting, including blocks with no retained index. |

P49 is the active medication action/unit/dose eligibility contract. Earlier conversion,
override, rate-only, summary-only, and `given`/`bolus` language is retained above as decision
history but is no longer operative. P43's mean/SD and median/IQR outputs remain.
...
code/
  ...
  03_context.py           index paralytic −30/+60 min → D; ± 5 min → E
...
```
  ...
  03_context.py
        │
        │  D  first non-IMV → IMV transition in t − 30/+60   (waterfall)
        │  E  sedatives in t ± 5, with dose and unit
        ▼
...
Every administration belongs to **exactly one formed** index event, so the formed set partitions the administration set and `Σ n_before_merge_admin` over all formed events equals the source administration count. Only after formation and same-medication merging does P51 exclude procedural anchors. `02` asserts both the fold partition and the formed = excluded + retained accounting.

### 6.4 `step02__index_paralytic.parquet` — the contract

One row per retained nonprocedural index paralytic, written to `output/intermediate_phi/`.
...
**Assertions on write:** `index_paralytic_id` unique; `span_minutes ≤ collapse_gap_minutes` for every formed row; formed source administrations equal the source count; formed counts equal procedural exclusions plus retained counts; `n_admins == n_agents == len(doses)`; retained `p_num` is contiguous from 1 within each block; no null in any non-`doses` column.
...
| `step02__procedural_index_exclusion_summary.csv` | formed, procedural-excluded, and retained counts overall and by mutually exclusive agent stratum |
...
For each index paralytic, scan `[t − imv_window_before_minutes, t + imv_window_after_minutes]` — inclusive at both ends — and keep the **earliest** transition inside it.
...
| `fig_D1__imv_transition_offset.csv` | 5-minute bins across the configured IMV window (`[−30, +60]`) → n |
...
Offset bins are `[−30,−25)`, … , `[55, 60]` — 18 bins, left-closed and right-open except the last, which is closed so an offset of exactly `+60` has a home.

**Assertions:** `-imv_window_before_minutes ≤ imv_offset_minutes ≤ imv_window_after_minutes` on every recorded transition; `imv_transition` true implies `imv_transition_dttm` and `imv_offset_minutes` non-null and `no_transition_reason` null, and false implies the converse.
...
new       tests/test_imv_transition.py
              ...
              selection when several transitions fall inside −30/+60, and each
